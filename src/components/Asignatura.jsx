...
let jason = require("./../data/ramos.json");//leemos la data del json que contiene los ramos de la malla

const Asignatura = () => {//definimos los estados a ocupar en la aplicación
  ...
  const [inputSemester, setInputSemester] = useState(""); // Controla el semestre al que se agregará la nueva materia

  const addCurrentSemesterSubject = (subject, image, sigla, semestre) => {//agregamos el ramo escogido al semestre indicado
    setCurrentSemesterSubjects([...currentSemesterSubjects, { subject, image, sigla, semestre }]);
  };

  const removeCurrentSemesterSubject = (subject) => {//eliminamos el ramo escogido del semestre indicado (no se alcanzó a implementar)
    setCurrentSemesterSubjects(currentSemesterSubjects.filter((s) => s.subject !== subject));
  };

  const addSubject = () => {//se agregan las características del ramo al archivo ramos.json, para así ser guardadas
    if(!image){
    ...
    if (subject && sigla && inputSemester) {
      const data = { subject, image, sigla, "semestre": inputSemester }; 

      fetch("http://localhost:3001/jsonramos", { 
        method: "POST",
        headers: {
  ...
  return (
    <div>
      <h2>Ramos según el Semestre:</h2>
      <Container>{/*desplegamos los semestres a elegir, donde se muestran los ramos asociados a cada semestre, por medio de la característica "semestre" en el json*/}
      <Form.Control as="select" value={selectedSemester} onChange={(e) => setSelectedSemester(e.target.value)}>
        ...
        <Row>{/*Agregamos las vistas de los nuevos ramos ingresados al semestre respectivo*/}
      ...
      <Form>
        <Form.Group>
          <Form.Label>Subject</Form.Label>{/*Asignamos al nuevo componente creado, los valores que ingresa el usuario para su nuevo ramo*/}
